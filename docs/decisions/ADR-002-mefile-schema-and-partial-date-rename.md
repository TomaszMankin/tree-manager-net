...
- **`PartialDate`** (renamed from `ExtendedDateTime`) is `readonly record struct (int? Day, int? Month, int? Year)`. Serializes via `PartialDateExtensions.ToSerializedString()` to `"DD|MM|YYYY"` with `XX` wildcards for unknown components.
